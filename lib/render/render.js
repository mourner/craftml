...
function createNewSolid1(element, $scope, parent){
    let solid = new Solid()
    solid.role = 'group'
    solid.id = _.get(element, 'attribs.id')
    solid.name = element.name
...
export default function render(element, $scope) {
    ...
    if (_.get(element,'attribs.c-repeat')){
        ...
        if (toks.length == 1){
            ...
            let solid = createNewSolid1(element, $scope, this)
            ...
            let cs = _.map(vs, (v,i) => {
                ...
                // // why was this code block moved outside createNewSolid?
                // solid.attribs = {}
                // _.forIn(element.attribs, (value, key) =>{
                //     try {
                //         solid.attribs[key] = $scope.resolve(value)
