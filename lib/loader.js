...
export function lazy(moduleId, context){
    ...
    return {
        ...
        get() {
            ...
            // resolve craftml:// -> http://[path-to-api]/....
            if (url.match(/^craftml:/)){
                let craftmlurl = url
                let p = url.match(/^craftml:\/\/(.*)/)[1]
                url = `${context.engine.api}/modules/${p}`
                console.log(`loading (lazy) ${moduleId}  -> ${craftmlurl} -> ${url}`)
            } else {
                console.log(`loading (lazy) ${moduleId}  -> ${url}`)
            }
...
function _loadFromCraftMLAsync(context, sid){
    ...
    let url = `${context.engine.api}/modules/${sid}`
...
function resolve(moduleId, context){
    ...
    if (ms = moduleId.match(/^\.\/(.*)/)){
        ...
        } else if (context.base.match(/^craftml:\//)){

            let p = context.base.match(/^craftml:\/\/(.*)/)[1]
            absbase = `${context.engine.api}/modules/${p}`
...
export function loadModule(moduleId, context) {
    ...
    // // resolve craftml:// -> http://[path-to-api]/....
    // if (url.match(/^craftml:/)){
    //     let craftmlurl = resolved
    //     let p = url.match(/^craftml:\/\/(.*)/)[1]
    //     url = `${context.engine.api}/modules/${p}`
    //     console.log(`loading ${moduleId}  -> ${craftmlurl} -> ${url}`)
    // } else {
    //     console.log(`loading ${moduleId}  -> ${url}`)
    // }

    console.log(`loading ${moduleId}  -> ${resolved}`)
    ...
    if (toks[0].match(/^@gist/)){
    ...
    } else if (moduleId.match(/^http:\/\/((cdn|dev|staging)\.)*craftml.io\//)) {
        ...
        return fs.readFileAsync(p, 'utf8')
            ...
            let url = `${context.engine.api}/modules/${moduleId}`
            return _getContentsPromise(context, url)
                .then(function(contents){
                    let c = {
                        path: url,
...
export function loadSrc(context, src, type) {
    type = type || 'utf8'

    var abspath
    if (src.match(/^http/)) {
        abspath = src
    } else if (context.base.match(/^http/)) {
        abspath = url.resolve(context.base, src)
    } else {
        abspath = path.resolve(context.base, src)
    }
