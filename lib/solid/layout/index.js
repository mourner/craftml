...
const twoGroupsMethods = [cut, intersect, solidify]
...
export default function layoutEval(code){
    ...
    var solids = _.filter(this.children, (c) => {            
            return (((c.role == 'group' || c.role =='csg' || c.role == 'cag') &&
                (c.size.x > 0 || c.size.y > 0 || c.size.z > 0))
                    || (c.role == '1d')) && c.style['display'] != 'none'
    })
    ...
    _.forEach(ops, op => {
        ...
        if (f) {
            ...
            throw `"${op.name}" is not a valid layout command`
        }
    })
    this.fitToChildren()
}
...
function parse_layout_commands(code){
    ...
    // var exprs = code.match(/\s*((select|lineup|align|center|distribute|reverse|cut|intersect)[XYZ]*)\(.*?\)\s*/g)
